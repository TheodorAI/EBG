...
def train(
    ...
    """
    step_count = 0
    losses_dict = {'iter_num': [], 'loss': []}  # Initialize a dictionary to store losses

    for iter_num in tqdm(range(max_iters), desc="Processing", unit="item"):
        ...
        # Append iteration number and loss to the dictionary
        losses_dict['iter_num'].append(iter_num)
        losses_dict['loss'].append(loss.item())

    # Write losses to CSV file
    csv_file_path = os.path.join(out_dir, 'losses.csv')
    with open(csv_file_path, mode='w') as csv_file:
        fieldnames = ['iter_num', 'loss']
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
        writer.writeheader()
        for i in range(len(losses_dict['iter_num'])):
            writer.writerow({'iter_num': losses_dict['iter_num'][i], 'loss': losses_dict['loss'][i]})
